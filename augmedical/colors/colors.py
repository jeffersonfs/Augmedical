...
import numpy as np
from skimage.color import (combine_stains, hpx_from_rgb, hsv2rgb, rgb2hsv,
                           rgb_from_hpx, separate_stains)
...
class Desaturation(ImageTransform):
    ...
    def apply(self, x):
        ...
        # change saturation
        x[:, :, 1] *= 1 - (random_float(self.min_desaturation,
                                        self.max_desaturation))

        # change brightness
        x[:, :, 2] *= 1 - (random_float(self.min_value_reduction,
                                        self.max_value_reduction))
...
class ChannelBleaching(ImageTransform):
    def __init__(
            self,
            p=0.3,
            min_bleach=0.1,
    ...
    def apply(self, x):
        ...
        selected_channel = (random.randint(0, x.shape[-1] - 1) if
                            self.force_channel is None else self.force_channel)
...
class StainShift(ImageTransform):
    ...
    def apply(self, x):
        ...
        x[..., selected_channel] = np.roll(x[..., selected_channel],
